...
def draw_box_label(frame, bbox_cv2, id, cls):
    ...
    font_size = 0.7
    font_color = (0, 0, 0)
    x1, y1, x2, y2 = bbox_cv2[0], bbox_cv2[1], bbox_cv2[2], bbox_cv2[3]
    cv2.rectangle(frame, (x1, y1), (x2, y2), box_color, 2)
    cv2.rectangle(frame, (x1, y1 - 25), (x2, y1), box_color, -1, 1)
    text_x = str(id) + ':' + get_cls(cls)
    cv2.putText(frame, text_x, (x1, y1 - 5), font, font_size, font_color, 1, cv2.LINE_AA)
    return frame
